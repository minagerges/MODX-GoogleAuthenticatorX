...
class EmailInstructions extends modProcessor
{
    ...
    public function process()
    {
        ...
        if ($gaSettings) {
            ...
            $body = $this->modx->lexicon('gax.notifyemail_body', [   /*modify array according to template*/
                'username' => $GA->UserName,
            ]);
            $body = '<html><body>' . $body . '</body></html>';
            if (!$user->sendEmail($body, ['subject' => $subject])) {
                return $this->failure($this->modx->lexicon('gax.emailfail') . $this->modx->mail->mailer->ErrorInfo);
            }
            return $this->success($this->modx->lexicon('gax.emailsuccess'));
        }

        return $this->failure($this->modx->lexicon('Failed loading User GAx data.'));
    }
}
