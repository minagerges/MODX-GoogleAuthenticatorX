...
if($loggeduser){
    ...
    if($sudo != True){
        ...
        $sentstatus = $scriptProperties['status'];
        if($sentstatus == 'ENABLED'){ $status = true; }
        if($sentstatus == 'DISABLED'){ $status = false; }
        if(!isset($status)){ return $modx->error->failure('Please click "Show secret" first!'); }
        ...
        return $modx->error->success('Changed.');
    }
}